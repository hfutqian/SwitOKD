...
def save_state(model, best_acc, flag=''):
    ...
    torch.save(state, 'models/resnet18_' + flag + '_best.pth.tar')
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(trainloader):
        ...
        epsilon2 = torch.exp(-1 * (t_label / (t_label + s_label)) )
        ...
        if ps_pt2 > delta2 and t_label2 < s_label2:
            ...
            loss_full2 = criterion(output2, target) + \
                        kl_div(output_ks2.detach(), output2, 1) + kl_div(output.detach(), output2, 1)
...
def test():
    ...
    if acc > best_acc:
        best_acc = acc
        save_state(model, best_acc, flag='full')
...
def test2():
    ...
    if acc > best_acc2:
        best_acc2 = acc
        save_state(model2, best_acc2, flag='full2')
...
def test_ks2():
    ...
    if acc > best_acc_ks2:
        best_acc_ks2 = acc
        save_state(model_ks2, best_acc_ks2, flag='ks2')
...
def adjust_learning_rate(optimizer, epoch):
    update_list = [140, 200, 250, 300, 350, 400, 450]
