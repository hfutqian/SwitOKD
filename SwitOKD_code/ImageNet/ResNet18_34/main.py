...
def train(train_loader, model_T, model_S, criterion, optimizer_T, optimizer_S, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        epsilon = torch.exp(-0.1 * t_label / (s_label+t_label))
        ...
        if i % args.print_freq == 0:
            ...
            print('epoch: ' + str(epoch) + ' T:criterion ' + str(criterion(output_T, target_var).item()) + ' kl_div: ' + str(kl_div(output_S.detach(), output_T, 1).item()))
            print('epoch: ' + str(epoch) + ' S:criterion ' + str(criterion(output_S, target_var).item()) + ' kl_div: ' + str(kl_div(output_T.detach(), output_S, 1).item()))
            
            print('epoch: ' + str(epoch) + ' T:entropy ' + str(compute_entropy(output_T, 1).item()))
            print('epoch: ' + str(epoch) + ' S:entropy ' + str(compute_entropy(output_S, 1).item()))

            print('flag:' + str(flag))

            print('s_label:' + str(s_label.item()))
            print('t_label:' + str(t_label.item()))
            print('ps_pt:' + str(ps_pt.item()))
